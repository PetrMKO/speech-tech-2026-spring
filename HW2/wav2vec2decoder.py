...
class Wav2Vec2Decoder:
    def __init__(
        ...
        self.lm_model: kenlm.Model | None = kenlm.Model(lm_model_path) if lm_model_path else None
        self.decode_methods = ["greedy", "beam", "beam_lm", "beam_lm_rescore"]
        self.beam_top_rescore = 5
    # -----------------------------------------------------------------------
    # Provided utility — do NOT modify
    # -----------------------------------------------------------------------
    ...
    def greedy_decode(self, logits: torch.Tensor) -> str:
        ...
        filtered_ids = [pred_ids[0]]
        current_token=pred_ids[0]
        ...
        filtered_ids.append(token)
        filtered_ids = [x for x in filtered_ids if x != self.blank_token_id]
        phrase = self._ids_to_text(token_ids=filtered_ids)
    ...
    def beam_search_decode(self, logits: torch.Tensor, return_beams: bool = False):
        ...
        def update_beams(beams, tensor):
            
            new_beam = {}
            
            for prefix, (p_nb, p_b) in beams.items():
                for index, log_prob in enumerate(tensor):

                    if index == self.blank_token_id:
                        new_p_nb, new_p_b = new_beam.get(
                            prefix, (float('-inf'), float('-inf'))
                        )

                        new_p_b = _log_add(new_p_b, p_nb + log_prob)
                        new_p_b = _log_add(new_p_b, p_b + log_prob)

                        new_beam[prefix] = (new_p_nb, new_p_b)
                    else:
                        # --- определяем новый prefix ---
                        if len(prefix) > 0 and prefix[-1] == index:
                            new_prefix = prefix
                        else:
                        ...
                        # если повтор
                        if len(prefix) > 0 and prefix[-1] == index:
                            # можно только из blank
                            new_p_nb = _log_add(new_p_nb, p_b + log_prob)
                        else:
                            # новый символ
                            new_p_nb = _log_add(new_p_nb, p_b + log_prob)
                            new_p_nb = _log_add(new_p_nb, p_nb + log_prob)

                        new_beam[new_prefix] = (new_p_nb, new_p_b)
        ...
        for t in log_probs:
            beam = update_beams(beams=beam, tensor=t)

        beams = [
            (list(prefix), _log_add(p_nb, p_b))
            for prefix, (p_nb, p_b) in beam.items()
        ]

        # сортируем ещё раз (на всякий случай)
        beams = sorted(beams, key=lambda x: x[1], reverse=True)

        if return_beams:
            return beams

        phrase = self._ids_to_text(token_ids=beams[0][0])
    ...
    def beam_search_with_lm(self, logits: torch.Tensor) -> str:
        ...
        def update_beams(beams, tensor):
            ...
            for prefix, (p_nb, p_b, lm_state) in beams.items():
                for index, log_prob in enumerate(tensor):

                    log_prob = log_prob.item()

                    next_lm_state = lm_state
                    
                    if index == self.blank_token_id:
                        new_p_nb, new_p_b, _ = new_beam.get(
                            prefix, (float('-inf'), float('-inf'), lm_state)
                        )

                        new_p_b = _log_add(new_p_b, p_nb + log_prob)
                        new_p_b = _log_add(new_p_b, p_b + log_prob)

                        new_beam[prefix] = (new_p_nb, new_p_b, next_lm_state)
                        ...
                        new_p_nb, new_p_b, _ = new_beam.get(
                            new_prefix, (float('-inf'), float('-inf'), lm_state)
                        )
                        ...
                        if len(prefix) > 0 and prefix[-1] == index:
                            ...
                            if self.vocab[index] == self.word_delimiter:

                                hyp_text = self._ids_to_text(new_prefix)
                                ...
                                next_lm_state = kenlm.State()
                                lm_score = self.lm_model.BaseScore(lm_state, last_word, next_lm_state)

                                lm_score = lm_score * math.log(10)

                                new_p_nb += self.alpha * lm_score + self.beta

                        new_beam[new_prefix] = (new_p_nb, new_p_b, next_lm_state)
        ...
        beams = [
            (list(prefix), _log_add(p_nb, p_b))
            for prefix, (p_nb, p_b, _) in beam.items()
        ]
    ...
    def lm_rescore(self, beams: List[Tuple[List[int], float]]) -> str:
        ...
        for token_ids, ctc_score in beams[:self.beam_top_rescore]:
            text = self._ids_to_text(token_ids)

            lm_score = self.lm_model.score(text, bos=True, eos=True)
            lm_score *= math.log(10)

            total_score = ctc_score + self.alpha * lm_score + self.beta * len(text.split())

            rescored.append((text, total_score))
        
        best = max(rescored, key=lambda x: x[1])
        return best[0]
    ...
    def decode(self, raw_logits: torch.Tensor, method: str = "greedy") -> str:
        """
...
def test(decoder: Wav2Vec2Decoder, audio_path: str, reference: str) -> object:
    import jiwer
    ...
    errors = {}
    for method in ["beam_lm_rescore"]: #["beam", "greedy",  "beam_lm", "beam_lm_rescore"]:
        try:
...
if __name__ == "__main__":
    ...
    for file_path, reference in test_samples:
        test(decoder, file_path, reference)